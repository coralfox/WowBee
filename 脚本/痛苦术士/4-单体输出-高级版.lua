...
local function init()
    ...
    smfl_hp_cost       = (smfl_hp[smfl_p_level] + p_Spirit * 1.5) or 0
    --生命分流获取法力值  (基础值+法强*0.5)*天赋等级加成
    smfl_mp_get        = ((smfl_mp[smfl_p_level] + p_Sp * 0.5) * smfl_rank) or 0
...
-- 构造函数
function SpellDelay:new(spell, delay, type, buffname)
    local instance = {}
    ...
    instance.delay = delay or select(7, GetSpellInfo(spell)) / 1000 + 0.1
    instance.type = type or "cast"
    instance.buffname = buffname
    instance.buff_check = false
    instance.time = BeeGetVariable("time")

    if buffname == nil then
        instance.buff_check = true
    else
        instance.buff_check = BeeUnitBuff(buffname) > 0
    end
...
-- 处理方法
function SpellDelay:handle()
    if not self.time and self.buff_check then
        if self.type == "cast" then
            BeeRun("/cast " .. self.spell)
    ...
    if self.time and GetTime() - self.time > self.delay then
        BeeRun("/cast " .. self.spell)
        BeeSetVariable("time", GetTime())
    end
end
...
-- 创建一个 SpellDelay 实例
local sd = SpellDelay:new("spell_name", 2, "cast", "buff_name")
...
local function spell_delay(spell, delay, type, buffname)
    if delay == nil then delay = select(7, GetSpellInfo(spell)) / 1000 + 0.1 end
    if type == nil then type = "cast" end

    local buff_check = false
    local time = BeeGetVariable("time")

    if buffname == nil then
        buff_check = true
    else
        buff_check = BeeUnitBuff(buffname) > 0
    end


    local buff_check = buffname ~= nil and buff_check

    if not time and buff_check then
        if type == "cast" then
            BeeRun("/cast " .. spell)
    ...
    if time and GetTime() - time > delay then
        BeeRun("/cast " .. spell)
        BeeSetVariable("time", GetTime())
    end
end
...
local function npc_check()
    local unit_flag = UnitClassification(src)
    local unit_n = "normal,trivial,minus"
    local unit_e = "rareelite,elite,rare"
    local unit_b = "worldboss"

    if UnitHealthMax(src) <= UnitHealthMax(p) * 2 and UnitLevel(src) <= UnitLevel(p) + 10 and BeeStringFind(unit_flag, unit_n)
    then
        return "小怪"
...
if BeeUnitHealth(p, "%") >= p_hp_3 and BeeIsRun("/cast 生命分流") and BeeUnitAffectingCombat() and BeePlayerBuffTime("生命分流") < 1 and unit_can and mod_can
then
    if BeeUnitMana(p, "%") <= p_mp_3 and auto_smfp_toggle then
        BeeRun("/cast 生命分流")
...
--发现敌人斩杀线,自动吸取灵魂
if BeeUnitHealth(src, "%") <= 25 and BeeTargetDeBuffTime("吸取灵魂") <= 8 and unit_can and mod_can
then
    BeeRun("/cast 吸取灵魂")
    return
end

if unit_can and mod_can and IsSpellInRange("痛苦无常") == 1 then
