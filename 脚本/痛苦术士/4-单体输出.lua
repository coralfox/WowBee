...
------------------Buff---------------------------------------
local function UnitBuffList(unit, Nameid, Categories) -- 获得指定目标buff列表
    if Unit == nil then
        Unit = "target";
    end
    if Nameid == nil then
...
if daduan_can and tar_castingTime - tar_time > 1 and tar_time > 1 and unit_can and mod_can then
    if BeeIsRun("/cast 痛苦缠绕", tar) and BeeSpellCD("痛苦缠绕") == 0 and IsSpellInRange("痛苦缠绕", tar) ==
        1 then
...
if unit_can and mod_can and IsSpellInRange("暗影箭") == 1 then
    -- 瞬发暗影箭
    if BeePlayerBuffTime("暗影冥思") > 0 and unit_can and mod_can then
    ...
    if BeeStringFind(npc_check(), "精英,首领,小怪") then
        ...
        if kz_toggle and BeeTargetDeBuffTime("恐惧") < kz_time and BeeIsRun("恐惧", tar) and BeeRange(tar) <=
            kz_range and not BeeStringFind(UnitCreatureType(tar), "亡灵") then

            if cur_castname ~= "恐惧" and not BeeStringFind(npc_check(), "小怪") then
                if debug then
                    print("恐惧-打断释放" .. cur_castname or false)
                end
                SpellStopTargeting() -- 停止瞄准
                SpellStopCasting() -- 停止施法
                spell_cast("恐惧", 1.5, "kj")
            end
        end
        ...
        if not cur_castname and BeeSpellCD("鬼影缠身") == 0 and BeeTargetDeBuffTime("鬼影缠身") < 3 and
            ...
            cur_castLeft = cur_castLeft or 0

            if BeeStringFind(cur_castname or false, "暗影箭,吸取生命,吸取灵魂") and cur_castLeft >
                BeeTargetDeBuffTime("鬼影缠身") and BeeTargetDeBuffTime("鬼影缠身") > 0 and
                not BeeStringFind(npc_check(), "小怪") then
                if debug then
                    print("鬼影缠身-打断释放" .. cur_castname or false)
                end
                SpellStopTargeting() -- 停止瞄准
                SpellStopCasting() -- 停止施法
            ...
            elseif not BeeStringFind(cur_castname or false, "暗影箭,吸取生命,吸取灵魂") and
        ...
        if BeeStringFind(cur_castname or false, "痛苦无常") and spell_lastcast == "痛苦无常" and
            BeeTargetDeBuffTime("痛苦无常") > 10 then
            if debug then
                print("痛苦无常-打断释放" .. cur_castname or false)
            end
            SpellStopCasting() -- 停止施法
            if debug then
                print("痛苦无常-打断释放" .. cur_castname or false)
