...
local function init()
    ...
    -- 当前生命分流等级
    local smfl_p_level = smfl_p_level = string.format("%d",string.sub(select(2, GetSpellName("生命分流")), -2))
...
local function init()
    ...
    -- 当前生命分流等级
    smfl_p_level = smfl_p_level = string.format("%d",string.sub(select(2, GetSpellName("生命分流")), -2))
...
if unit_can and mod_can and IsSpellInRange("暗影箭") == 1 then
    if BeeStringFind(npc_check(), "精英,首领,小怪") then
        ...
        if BeeSpellCD("鬼影缠身") == 0 and BeeTargetDeBuffTime("鬼影缠身") < 4 and
            BeeIsRun("/cast 鬼影缠身") and unit_can and mod_can then
            if BeeStringFind(cur_casting, "暗影箭,吸取生命,吸取灵魂") and cur_time >
                BeeTargetDeBuffTime("鬼影缠身") and BeeTargetDeBuffTime("鬼影缠身") > 0 then
                if debug then
        ...
        if BeeSpellCD("痛苦无常") == 0 and BeeTargetDeBuffTime("痛苦无常") < 4 and
            ...
            if BeeStringFind(cur_casting, "暗影箭,吸取生命,吸取灵魂") and cur_time > 1 and cur_time + 1.5 >
                BeeTargetDeBuffTime("痛苦无常") and BeeTargetDeBuffTime("痛苦无常") > 0 then
                if debug then
