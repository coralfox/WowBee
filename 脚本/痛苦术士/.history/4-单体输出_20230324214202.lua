...
if BeeUnitHealth(p, "%") >= p_hp_3 and BeeUnitMana(p, "%") <= p_mana_3 and smfl_dw and BeeIsRun("/cast 生命分流") and
    ...
    if auto_smfp_toggle then
        for i = smfl_p_level, 1, -1 do
            ...
            if () > (smfl_safe_hp / 100 * UnitHealthMax(p)) and
...
if unit_can and mod_can and IsSpellInRange("暗影箭") == 1 then
    if BeeStringFind(npc_check(), "精英,首领,小怪") then
        ...
        if BeeSpellCD("痛苦无常") == 0 and
            (BeeSpellCD("鬼影缠身") > 0 or BeeTargetDeBuffTime("鬼影缠身") >= 3) and
            ...
            if BeeStringFind(cur_casting, "暗影箭,吸取生命,吸取灵魂") and cur_time > 1 and cur_time + 1.5 >
                ...
                SpellStopCasting() -- 停止施法
                spell_cast("鬼影缠身", 0.3, "gycs")
            elseif BeeStringFind(cur_casting, "痛苦无常") and BeeTargetDeBuffTime("痛苦无常") > 0 then
                SpellStopCasting() -- 停止施法
            else
