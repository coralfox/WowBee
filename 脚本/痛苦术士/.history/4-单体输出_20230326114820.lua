...
local function spell_cast(name, delay, s_name)
    ...
    if not lastcast or GetTime() - lastcast > casttime then
        BeeRun("/cast " .. name)
        print("施放技能:" .. name .. " 延迟:" .. delay .. "s" .. "\r施法时间:" .. casttime .. "s")
...
if unit_can and mod_can and IsSpellInRange("暗影箭") == 1 then
    ...
    if BeeStringFind(npc_check(), "精英,首领,小怪") then
        ...
        if not cur_castname and (BeeSpellCD("鬼影缠身") > 1.5 or BeeTargetDeBuffTime("鬼影缠身") >= 3) and
        ...
        -- local cur_time, cur_castingTime = BeeUnitCastSpellTime(p) -- 当前施法时间
        if (BeeSpellCD("鬼影缠身") > 2 or BeeTargetDeBuffTime("鬼影缠身") >= 3) and
            ...
            if debug then
                ...
                print("正在释放" .. cur_castname)
            end

            -- print("鬼影缠身的CD:" .. BeeSpellCD("鬼影缠身") .. "\r鬼影缠身buff时间:" ..
            --           BeeTargetDeBuffTime("鬼影缠身") .. "\r痛苦无常buff时间:" ..
            --           BeeTargetDeBuffTime("痛苦无常") .. "\r当前施法:" .. cur_castname ..
