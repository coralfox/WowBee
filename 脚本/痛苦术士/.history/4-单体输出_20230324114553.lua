...
local cur_casting, cur_time, cur_castingTime = BeeUnitCastSpellTime(p) -- 当前施法
...
if unit_can and mod_can and IsSpellInRange("暗影箭") == 1 then
    if BeeStringFind(npc_check(), "精英,首领,小怪") then
        ...
        if BeeSpellCD("鬼影缠身") == 0 and BeeTargetDeBuffTime("鬼影缠身") < 4 and
            ...
            if BeeStringFind(cur_casting, "暗影箭,吸取生命,吸取灵魂,火焰之雨,腐蚀之种") and cur_time >
                BeeTargetDeBuffTime("鬼影缠身") then
        ...
        if not cur_casting and BeeSpellCD("痛苦无常") == 0 and BeeTargetDeBuffTime("痛苦无常") < 4 and
            BeeIsRun("/cast 痛苦无常") and cur_time > BeeTargetDeBuffTime("鬼影缠身") and unit_can and mod_can then
            if BeeStringFind(cur_casting, "暗影箭,吸取生命,吸取灵魂,火焰之雨,腐蚀之种") then
                SpellStopTargeting() -- 停止瞄准
                SpellStopCasting() -- 停止施法
            end
            if debug then
        ...
        if not cur_casting and BeeUnitHealth(p, "%") >= p_hp_3 and BeeUnitMana(p, "%") <= p_mana_3 and
