...
if unit_can and mod_can and IsSpellInRange("暗影箭") == 1 then
    ...
    if BeeStringFind(npc_check(), "精英,首领,小怪") then
        if BeeIsRun("/petattack") and unit_can and mod_can then
            ...
            local s_name = "pet_a"
            local pet_cast = BeeGetVariable(s_name .. "_pet_cast")
            if not pet_cast or GetTime() - pet_cast > casttime then
                BeeRun("/petattack")
                BeeSetVariable(s_name .. "_pet_cast", GetTime())
        ...
        local cur_time, cur_castingTime = BeeUnitCastSpellTime(p) -- 当前施法时间
        if BeeSpellCD("鬼影缠身") > 1 and BeeTargetDeBuffTime("痛苦无常") < 4 and BeeIsRun("/cast 痛苦无常") and
