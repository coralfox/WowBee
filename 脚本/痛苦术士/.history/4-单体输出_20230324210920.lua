...
local function init()
    ...
    -- 当前生命分流等级
    local smfl_p_level = tonumber(string.sub(select(2, GetSpellName("生命分流")), -2))
...
if unit_can and mod_can and IsSpellInRange("暗影箭") == 1 then
    if BeeStringFind(npc_check(), "精英,首领,小怪") then
        ...
        if not cur_casting and (BeeSpellCD("鬼影缠身") > 1.5 or BeeTargetDeBuffTime("鬼影缠身") >= 3) and
        ...
        if BeeSpellCD("痛苦无常") == 0 and
            (BeeSpellCD("鬼影缠身") > 1.5 or BeeTargetDeBuffTime("鬼影缠身") >= 3) and
            BeeTargetDeBuffTime("痛苦无常") < 4 and BeeIsRun("/cast 痛苦无常") and unit_can and mod_can then
            if BeeStringFind(cur_casting, "暗影箭,吸取生命,吸取灵魂") and cur_time > 1 and cur_time + 1.5 >
                BeeTargetDeBuffTime("痛苦无常") and BeeTargetDeBuffTime("痛苦无常") > 0 and
                not BeeStringFind(npc_check(), "小怪") then
                if debug then
                    print("痛苦无常-打断释放" .. cur_casting)
                end
                SpellStopTargeting() -- 停止瞄准
                SpellStopCasting() -- 停止施法
            elseif BeeStringFind(cur_casting, "痛苦无常") and -- 鬼影缠身的持续时间-当前释放剩余时间-当前飞行时间-鬼影缠身的施法时间>0
            BeeTargetDeBuffTime("鬼影缠身") - cur_time - cur_delay - select(7, GetSpellInfo("鬼影缠身")) / 1000 >
                0 and BeeTargetDeBuffTime("鬼影缠身") > 0 then
                spell_cast("鬼影缠身", 0.3, "gycs")
            end
        ...
        if not cur_casting and BeeTargetDeBuffTime("痛苦无常") >= 4 and BeeSpellCD("痛苦诅咒") == 0 and
        ...
        if not cur_casting and (BeeSpellCD("鬼影缠身") > 1.5 or BeeTargetDeBuffTime("鬼影缠身") >= 4) and
