...
-- 目标身上有控制BUFF，不要捣乱
local ctr_buff = "致盲,凿击,昏迷,变形术,闷棍,致盲,圣盾术,保护之手" -----此为判定目标身上BUFF停手
local joylau =
...
local self_buff = "猛踢,假死,机械路霸,X-53型观光火箭,奥的灰烬,点心,进食,喝水" -----坐骑和自身有BUFF停手
...
-- 目标有控制buff，自身有吃喝，坐骑等buff，自身正在施法，不要打断
if BeeStringFind(ctr_buff, Tbl) or BeeStringFind(self_buff, buff) or selfStopCastSpell then
    return
end

local unit_can = unit_check(tar)

if daduan_can and tar_Castingtime - tar_time > 1 and tar_time > 1 then
    if BeeIsRun("/cast 痛苦缠绕", tar) and BeeSpellCD("痛苦缠绕") == 0 and IsSpellInRange("痛苦缠绕", src) ==
        1 then
        BeeRun("痛苦缠绕", tar)
        return
    end
    if BeeIsRun("/cast 恐惧", tar) and BeeSpellCD("致盲") == 0 and IsSpellInRange("致盲", a) == 1 and
        BeeSpellCD("脚踢") > 0 then
        BeeRun("致盲", a)
        return
    end
end
...
if BeeUnitHealth(p, "%") >= p_hp_3 and smfl_dw and BeeIsRun("/cast 生命分流") and BeeUnitAffectingCombat() and
    BeePlayerBuffTime("生命分流") < 1 and unit_can and mod_can then
...
if unit_can and mod_can and IsSpellInRange("暗影箭") == 1 and not cur_casting then
    if BeeStringFind(npc_check(), "精英,首领") then
        -- 输出
        if not cur_casting and BeeSpellCD("暗影箭") == 0 and BeeTargetDeBuffTime("暗影掌握") < 1 and
            BeeIsRun("/cast 暗影箭") and unit_can and mod_can then
            spell_cast("暗影箭", 0.3, "ayj")
            -- BeeRun("/cast 吸取生命")
        end
        if BeeTargetDeBuffTime("鬼影缠身") < 2 and BeeIsRun("/cast 鬼影缠身") and unit_can and mod_can then
        ...
        if BeeTargetDeBuffTime("痛苦无常") < 4 and BeeIsRun("/cast 痛苦无常") and unit_can and mod_can then
        ...
        if BeeTargetDeBuffTime("鬼影缠身") >= 4 and BeeTargetDeBuffTime("痛苦无常") >= 4 and
            BeeTargetDeBuffTime("痛苦诅咒") >= 6 and BeeUnitHealth(tar, "%") > 40 and BeeUnitHealth(tar) > 1500 and
            BeeIsRun("/cast 暗影箭") and unit_can and mod_can then
        ...
        if BeeTargetDeBuffTime("鬼影缠身") >= 4 and BeeTargetDeBuffTime("痛苦无常") >= 4 and
            ...
            BeeIsRun("/cast 吸取生命") and BeeUnitHealth(p, "%") <= keep_hp and unit_can and mod_can then
    ...
    elseif npc_check() == "小怪" then
        ...
        if BeeTargetDeBuffTime("痛苦无常") >= 4 and BeeTargetDeBuffTime("痛苦诅咒") < 6 and
            BeeIsRun("/cast 痛苦诅咒") and unit_can and mod_can then
            spell_cast("痛苦诅咒", 0.2, "tkjz")
            -- BeeRun("/cast 痛苦诅咒")
        end

        if BeeTargetDeBuffTime("鬼影缠身") >= 4 and BeeTargetDeBuffTime("痛苦诅咒") >= 6 and
            BeeUnitHealth(tar, "%") > 40 and BeeUnitHealth(tar) > 1500 and BeeIsRun("/cast 暗影箭") and unit_can and
            mod_can then
            spell_cast("暗影箭", 0.2, "ayj")
            -- BeeRun("/cast 暗影箭")
            return
