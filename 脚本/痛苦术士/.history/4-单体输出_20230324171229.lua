...
if unit_can and mod_can and IsSpellInRange("暗影箭") == 1 then
    if BeeStringFind(npc_check(), "精英,首领,小怪") then
        ...
        if BeeSpellCD("鬼影缠身") == 0 and BeeTargetDeBuffTime("鬼影缠身") < 3 and
            ...
            local delay = tar_range / 30 * 1.5
            -- 服务器延迟加上当前暗影箭的飞行时间
            local cur_delay = 鬼影缠身 + server_delay
            ...
            --鬼影缠身的持续时间-当前释放剩余时间-当前飞行时间-鬼影缠身的持续时间>0
                BeeTargetDeBuffTime("鬼影缠身") - cur_time - cur_delay - select(7, GetSpellInfo("鬼影缠身")) / 1000>0
                and BeeTargetDeBuffTime("鬼影缠身") > 0 and
                BeeStringFind(npc_check(), "小怪") then
                if debug then
                    print("鬼影缠身-打断释放" .. cur_casting)
                end
                SpellStopTargeting() -- 停止瞄准
        ...
        if not cur_casting and BeeTargetDeBuffTime("痛苦无常") >= 3 and BeeSpellCD("痛苦诅咒") == 0 and
