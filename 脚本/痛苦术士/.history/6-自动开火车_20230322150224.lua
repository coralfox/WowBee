...
if BeeUnitHealth("player", "%") >= p_hp_3 and BeeUnitMana("player", "%") <= p_mana_3 and BeeIsRun("/cast 生命分流") and unit_can and mod_can
then
    ...
    if not xmfl_lastcast or GetTime() - xmfl_lastcast > xmfl_casttime
    then
...
--释放腐蚀术
if BeeIsRun("/cast 腐蚀术", src) and IsSpellInRange("腐蚀术", src) == 1 and not BeeStringFind("腐蚀术", Tbl) and mod_can then
    BeeRun("/cast 腐蚀术", src)
end

--释放痛苦诅咒
if BeeIsRun("/cast 痛苦诅咒", src) and IsSpellInRange("痛苦诅咒", src) == 1 and not BeeStringFind("痛苦诅咒", Tbl) and mod_can then
