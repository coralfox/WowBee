...
--#################自动回血(宝宝)##################
--不在战斗状态则自动回血
if BeeUnitMana("pet", nil) >=GetSpellPowerCost("吞噬暗影") and BeeUnitHealth("pet", "%") <= pet_hp and BeeIsRun("/cast 吞噬暗影", "pet") and not BeeUnitAffectingCombat("pet") and not IsMounted(p) and BeeSpellCD("吞噬暗影") == 0
then
