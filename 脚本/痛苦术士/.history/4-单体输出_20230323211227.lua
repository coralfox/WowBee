...
if unit_can and mod_can and IsSpellInRange("暗影箭") == 1 then
    ...
    elseif npc_check() == "小怪" then
        ...
        if BeeTargetDeBuffTime("鬼影缠身") >= 2 and BeeTargetDeBuffTime("痛苦无常") >= 4 and BeeTargetDeBuffTime("痛苦诅咒") >= 6 and BeeUnitHealth(src, "%") > 40 and BeeUnitHealth(src) > 1500 and BeeIsRun("/cast 暗影箭") and unit_can and mod_can
        then
            spell_cast("暗影箭",0.2,"ayj")
            -- BeeRun("/cast 暗影箭")
            return
        ...
        if BeeTargetDeBuffTime("鬼影缠身") >= 2 and BeeTargetDeBuffTime("痛苦无常") >= 4 and BeeTargetDeBuffTime("痛苦诅咒") >= 6 and BeeUnitHealth(src, "%") > 25 and BeeUnitHealth(src) > 800 and BeeIsRun("/cast 吸取生命") and BeeUnitHealth(p, "%") <= keep_hp and unit_can and mod_can
        then
            spell_cast("吸取生命",0.2,"xqsh")
            -- BeeRun("/cast 吸取生命")
            return
