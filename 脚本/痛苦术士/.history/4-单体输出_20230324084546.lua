...
local i = BeeStringFind(jineng,BeeUnitCastSpellName(tar))
...
local function npc_check()
    ...
    if UnitHealthMax(tar) <= UnitHealthMax(p) * 0.5 and UnitLevel(tar) <= UnitLevel(p) and BeeStringFind(unit_flag, unit_n)
    then
        return "杂碎"
    elseif UnitHealthMax(tar) <= UnitHealthMax(p) * 2 and UnitLevel(tar) <= UnitLevel(p) + 10 and BeeStringFind(unit_flag, unit_n)
    then
        return "小怪"
    elseif UnitHealthMax(tar) <= 4 * UnitHealthMax(p) and UnitLevel(tar) <= UnitLevel(p) + 10 and BeeStringFind(unit_flag, unit_e)
    then
        return "精英"
    elseif UnitHealthMax(tar) >= 4 * UnitHealthMax(p) and UnitLevel(tar) <= UnitLevel(p) + 10 and (BeeStringFind(unit_flag, unit_b) or BeeStringFind(unit_flag, unit_e))
    then
        return "首领"
    end
end
...
--发现敌人斩杀线,自动吸取灵魂
if BeeUnitHealth(tar, "%") <= 25 and BeeTargetDeBuffTime("吸取灵魂") <= 4 and unit_can and mod_can
then
...
if BeeStringFind(npc_check(), "精英,首领") then
    --输出
    if not cur_casting and BeeSpellCD("暗影箭")==0 and BeeTargetDeBuffTime("暗影掌握")< 1 and BeeIsRun("/cast 暗影箭") and unit_can and mod_can
        then
            spell_cast("暗影箭",0.3,"ayj")
        -- BeeRun("/cast 吸取生命")
        end
        if BeeTargetDeBuffTime("鬼影缠身") < 2 and BeeIsRun("/cast 鬼影缠身")  and unit_can and mod_can
        then
        ...
        if BeeTargetDeBuffTime("腐蚀术") < 3 and BeeIsRun("/cast 腐蚀术") and unit_can and mod_can
        then
            -- spell_cast("腐蚀术",0.2,"fss")
            BeeRun("腐蚀术")
        end

        if BeeTargetDeBuffTime("痛苦无常") < 4  and BeeIsRun("/cast 痛苦无常") and unit_can and mod_can
        then
        ...
        if  BeeTargetDeBuffTime("痛苦诅咒") < 6 and BeeIsRun("/cast 痛苦诅咒")  and unit_can and mod_can
        then
            spell_cast("痛苦诅咒",0.2,"tkjz")
            -- BeeRun("/cast 痛苦诅咒")
        end

        if BeeTargetDeBuffTime("鬼影缠身") >= 4 and BeeTargetDeBuffTime("痛苦无常") >= 4 and BeeTargetDeBuffTime("痛苦诅咒") >= 6 and BeeUnitHealth(tar, "%") > 40 and BeeUnitHealth(tar) > 1500 and BeeIsRun("/cast 暗影箭") and unit_can and mod_can
        then
            spell_cast("暗影箭",0.2,"ayj")
            -- BeeRun("/cast 暗影箭")
        end

        if BeeTargetDeBuffTime("鬼影缠身") >= 4 and BeeTargetDeBuffTime("痛苦无常") >= 4 and BeeTargetDeBuffTime("痛苦诅咒") >= 6 and BeeUnitHealth(tar, "%") > 25 and BeeUnitHealth(tar) > 800 and BeeIsRun("/cast 吸取生命") and BeeUnitHealth(p, "%") <= keep_hp and unit_can and mod_can
        then
    ...
    elseif npc_check() == "小怪" then
        ...
        if BeeTargetDeBuffTime("痛苦无常") >= 4  and BeeTargetDeBuffTime("痛苦诅咒") < 6 and BeeIsRun("/cast 痛苦诅咒")  and unit_can and mod_can
        then
            spell_cast("痛苦诅咒",0.2,"tkjz")
            -- BeeRun("/cast 痛苦诅咒")
        end

        if BeeTargetDeBuffTime("鬼影缠身") >= 4  and BeeTargetDeBuffTime("痛苦诅咒") >= 6 and BeeUnitHealth(tar, "%") > 40 and BeeUnitHealth(tar) > 1500 and BeeIsRun("/cast 暗影箭") and unit_can and mod_can
        then
            spell_cast("暗影箭",0.2,"ayj")
            -- BeeRun("/cast 暗影箭")
            return
        ...
        if BeeTargetDeBuffTime("鬼影缠身") >= 4 and BeeTargetDeBuffTime("痛苦无常") >= 4 and BeeTargetDeBuffTime("痛苦诅咒") >= 6 and BeeUnitHealth(tar, "%") > 25 and BeeUnitHealth(tar) > 800 and BeeIsRun("/cast 射击") and unit_can and mod_can
        then
            spell_cast("射击",3,"sj")
            return
        end
    elseif npc_check() == "杂碎" then
        ...
        if BeeTargetDeBuffTime("痛苦诅咒") >= 6 and BeeUnitHealth(tar, "%") > 25  and BeeIsRun("/cast 射击") and unit_can and mod_can
        then
            spell_cast("射击",4,"sj")
            return
        end
