...
local function init()
    ...
    -- 当前生命分流等级
    smfl_p_level = string.format("%d", string.sub(select(2, GetSpellName("生命分流")), -2))
...
-- 初次运行初始化
local first_init = BeeGetVariable("buff_first_init")

if not first_init then
    BeeSetVariable("buff_first_init", true)
    init()
end
...
if BeePlayerBuffTime("魔甲术") < 1 and BeeUnitMana(p, "%", 0) >= p_mana and BeeIsRun("/cast 魔甲术") and mod_can then
    BeeRun("/cast 魔甲术")
    return
end
-- #################自动回蓝(生命分流)##################
