...
--起手,腐蚀术
local spell_cast(name,delay,)
    local cur_spell = name
    local random_delay = 0.21

    local fss_casttime = select(7, GetSpellInfo(fss_spell)) / 1000 + random_delay
    local fss_lastcast = BeeGetVariable("fss_lastcast")
    if not fss_lastcast or GetTime() - fss_lastcast > fss_casttime
    then
        BeeRun("/cast " .. fss_spell)
        BeeSetVariable("fss_lastcast", GetTime())
    end
    return
    BeeRun("/cast 腐蚀术")
    return
