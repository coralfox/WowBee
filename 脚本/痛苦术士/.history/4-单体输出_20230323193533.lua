...
--起手
local spell_cast(name,delay,s_name,order)

    local casttime = select(7, GetSpellInfo(name)) / 1000 + delay
    local lastcast = BeeGetVariable(s_name.."lastcast")
    if not lastcast or GetTime() - lastcast > casttime
    then
        BeeRun("/cast " .. name)
        BeeSetVariable(s_name.."lastcast", GetTime())
        BeeSetVariable(s_name.."order", GetTime())
    end
    return
end
