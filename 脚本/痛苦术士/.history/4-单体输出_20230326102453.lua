...
local spell_castname = BeeUnitCastSpellName(p) -- 当前施法
...
local function spell_casting(unit)
    ...
    if spell then
        ...
        return spell, castTime, castingTime, castLeft
    else
        return false
    end
end
...
if unit_can and mod_can and IsSpellInRange("暗影箭") == 1 then
    ...
    if BeeStringFind(npc_check(), "精英,首领,小怪") then
        ...
        -- 输出
        if not spell_castname and BeeSpellCD("暗影箭") == 0 and BeeTargetDeBuffTime("暗影之拥") < 1 and
        ...
        if BeeSpellCD("鬼影缠身") == 0 and BeeTargetDeBuffTime("鬼影缠身") < 3 and
            ...
            if BeeStringFind(BeeUnitCastSpellName(p), "暗影箭,吸取生命,吸取灵魂") and cur_time >
                BeeTargetDeBuffTime("鬼影缠身") and BeeTargetDeBuffTime("鬼影缠身") > 0 and
                not BeeStringFind(npc_check(), "小怪") then
                if debug then
                    print("鬼影缠身-打断释放" .. BeeUnitCastSpellName(p))
                end
                SpellStopTargeting() -- 停止瞄准
                SpellStopCasting() -- 停止施法
                spell_cast("鬼影缠身", 0.3, "gycs")
            elseif BeeStringFind(BeeUnitCastSpellName(p), "暗影箭,吸取生命,吸取灵魂") and
                -- 鬼影缠身的持续时间-当前释放剩余时间-当前飞行时间-鬼影缠身的施法时间>0
                BeeTargetDeBuffTime("鬼影缠身") - cur_time - cur_delay - select(7, GetSpellInfo("鬼影缠身")) /
        ...
        if not BeeUnitCastSpellName(p) and
            (BeeSpellCD("鬼影缠身") > 1.5 or BeeTargetDeBuffTime("鬼影缠身") >= 3) and BeeSpellCD("腐蚀术") ==
            0 and BeeTargetDeBuffTime("腐蚀术") < 3 and BeeIsRun("/cast 腐蚀术") and unit_can and mod_can then
        ...
        local cur_time, cur_castingTime = BeeUnitCastSpellTime(p) -- 当前施法时间
        if (BeeSpellCD("鬼影缠身") > 0 or BeeTargetDeBuffTime("鬼影缠身") >= 3) and
            BeeTargetDeBuffTime("痛苦无常") < 4 and BeeIsRun("/cast 痛苦无常") and unit_can and mod_can then
            if BeeStringFind(BeeUnitCastSpellName(p), "暗影箭,吸取生命,吸取灵魂") and cur_time > 1 and
                cur_time + 1.5 > BeeTargetDeBuffTime("痛苦无常") and BeeTargetDeBuffTime("痛苦无常") > 0 and
                not BeeStringFind(npc_check(), "小怪") then
                if debug then
                    print("痛苦无常-打断释放" .. BeeUnitCastSpellName(p))
                end
                SpellStopTargeting() -- 停止瞄准
                SpellStopCasting() -- 停止施法
                spell_cast("痛苦无常", 0.5, "tkwc")
            elseif BeeStringFind(BeeUnitCastSpellName(p), "痛苦无常") and BeeTargetDeBuffTime("痛苦无常") > 0 then
                SpellStopCasting() -- 停止施法
                if debug then
                    print("痛苦无常-打断释放" .. BeeUnitCastSpellName(p))
                end
            ...
            if debug then
                -- print("鬼影缠身的CD:" .. BeeSpellCD("鬼影缠身") .. "\r鬼影缠身buff时间:" ..
                --           BeeTargetDeBuffTime("鬼影缠身") .. "\r痛苦无常buff时间:" ..
                --           BeeTargetDeBuffTime("痛苦无常"))
                print("正在释放" .. BeeUnitCastSpellName(p))
            end
            -- print("鬼影缠身的CD:" .. BeeSpellCD("鬼影缠身") .. "\r鬼影缠身buff时间:" ..
            --           BeeTargetDeBuffTime("鬼影缠身") .. "\r痛苦无常buff时间:" ..
            --           BeeTargetDeBuffTime("痛苦无常") .. "\r当前施法:" .. BeeUnitCastSpellName(p) ..
            --           "\r当前施法时间:" .. cur_time .. "\r当前施法剩余时间:" .. cur_castingTime)
            -- print("痛苦无常运行中" .. GetTime())
        end

        if not BeeUnitCastSpellName(p) and BeeTargetDeBuffTime("痛苦无常") >= 4 and BeeSpellCD("痛苦诅咒") == 0 and
            BeeTargetDeBuffTime("痛苦诅咒") < 6 and BeeIsRun("/cast 痛苦诅咒") and unit_can and mod_can then
            spell_cast("痛苦诅咒", 0.2, "tkjz")
            -- BeeRun("/cast 痛苦诅咒")
        end

        if BeeUnitCastSpellName(p) ~= "生命吸取" and BeeSpellCD("生命吸取") == 0 and
            BeeTargetDeBuffTime("鬼影缠身") >= 4 and BeeTargetDeBuffTime("痛苦无常") >= 4 and
            BeeTargetDeBuffTime("痛苦诅咒") >= 6 and BeeUnitHealth(tar, "%") > 25 and BeeIsRun("/cast 吸取生命") and
            BeeUnitHealth(p, "%") <= keep_hp and unit_can and mod_can then
            spell_cast("吸取生命", 0.2, "xqsh")
            -- BeeRun("/cast 吸取生命")
        end

        if not BeeUnitCastSpellName(p) and
            (BeeSpellCD("鬼影缠身") > 1.5 or BeeTargetDeBuffTime("鬼影缠身") >= 4) and
        ...
        if not BeeUnitCastSpellName(p) and (BeeSpellCD("鬼影缠身") > 4 or BeeTargetDeBuffTime("鬼影缠身") >= 4) and
            BeeTargetDeBuffTime("痛苦无常") >= 4 and BeeTargetDeBuffTime("痛苦诅咒") >= 6 and
            BeeUnitHealth(tar, "%") > 40 and BeeIsRun("/cast 暗影箭") and unit_can and mod_can then
        ...
        if BeeStringFind(npc_check(), "小怪") then
            if not BeeUnitCastSpellName(p) and BeeTargetDeBuffTime("鬼影缠身") >= 4 and
                BeeTargetDeBuffTime("痛苦无常") >= 4 and BeeTargetDeBuffTime("痛苦诅咒") >= 6 and
                BeeUnitHealth(tar, "%") > 25 and BeeUnitHealth(tar) > 800 and BeeIsRun("/cast 射击") and unit_can and
                mod_can then
                spell_cast("射击", 3, "sj")
                return
            end
