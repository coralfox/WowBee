...
local cur_casting                                  = BeePlayerCasting() --当前施法
...
if not BeeUnitCastSpellName(p)  and BeeTargetDeBuffTime("暗影掌握")<=0 and BeeIsRun("/cast 暗影箭") and unit_can and mod_can
then
...
if unit_can and mod_can and IsSpellInRange("暗影箭") == 1 and not BeeUnitCastSpellName(p)  and then
 ...
 if BeeTargetDeBuffTime("鬼影缠身") >= 4 and BeeTargetDeBuffTime("痛苦无常") >= 4 and BeeTargetDeBuffTime("痛苦诅咒") >= 6 and BeeUnitHealth(src, "%") > 40 and BeeUnitHealth(src) > 1500 and BeeIsRun("/cast 暗影箭") and unit_can and mod_can
 then
     spell_cast("暗影箭",0.2,"ayj")
     -- BeeRun("/cast 暗影箭")
 end

 if BeeTargetDeBuffTime("鬼影缠身") >= 4 and BeeTargetDeBuffTime("痛苦无常") >= 4 and BeeTargetDeBuffTime("痛苦诅咒") >= 6 and BeeUnitHealth(src, "%") > 25 and BeeUnitHealth(src) > 800 and BeeIsRun("/cast 吸取生命") and BeeUnitHealth(p, "%") <= keep_hp and unit_can and mod_can
 then
    ...
    elseif npc_check() == "小怪" then
        ...
        if BeeTargetDeBuffTime("鬼影缠身") >= 4  and BeeTargetDeBuffTime("痛苦诅咒") >= 6 and BeeUnitHealth(src, "%") > 40 and BeeUnitHealth(src) > 1500 and BeeIsRun("/cast 暗影箭") and unit_can and mod_can
        then
            spell_cast("暗影箭",0.2,"ayj")
            -- BeeRun("/cast 暗影箭")
            return
        ...
        if BeeTargetDeBuffTime("鬼影缠身") >= 4 and BeeTargetDeBuffTime("痛苦无常") >= 4 and BeeTargetDeBuffTime("痛苦诅咒") >= 6 and BeeUnitHealth(src, "%") > 25 and BeeUnitHealth(src) > 800 and BeeIsRun("/cast 射击") and unit_can and mod_can
        then
            spell_cast("射击",3,"sj")
            return
        end
    elseif npc_check() == "杂碎" then
        ...
        if BeeTargetDeBuffTime("痛苦诅咒") >= 6 and BeeUnitHealth(src, "%") > 25  and BeeIsRun("/cast 射击") and unit_can and mod_can
        then
            spell_cast("射击",4,"sj")
            return
        end
