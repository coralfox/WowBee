...
if unit_can and mod_can and IsSpellInRange("暗影箭") == 1 then
    ...
    elseif npc_check() == "小怪" then
        --输出
        if   BeeUnitHealth(src, "%") > 25 and BeeUnitHealth(src) > 800 and BeeIsRun("/cast 吸取生命") and BeeUnitHealth(p, "%") <= keep_hp and unit_can and mod_can
        then
            spell_cast("吸取生命",0.2,"xqsh")
            -- BeeRun("/cast 吸取生命")
            return
        ...
        if BeeTargetDeBuffTime("鬼影缠身") < 4 and unit_can and mod_can
        then
