...
if BeeSpellCD("生命分流") == 0 and BeeUnitHealth(p, "%") >= p_hp_3 and BeeUnitMana(p, "%") <= p_mana_3 and
...
-- 瞬发暗影箭
if BeeSpellCD("暗影箭") == 0 and BeePlayerBuffTime("暗影冥想") > 0 and IsSpellInRange("暗影箭") == 1 and
...
-- 释放腐蚀术
if BeeSpellCD("腐蚀术") == 0 and BeeIsRun("/cast 腐蚀术", src) and IsSpellInRange("腐蚀术", src) == 1 and
    not BeeStringFind("腐蚀术", Tbl) and mod_can then
    spell_cast("腐蚀术", 1.1, "fss")
    -- BeeRun("/cast 腐蚀术", src)
end

-- 释放痛苦诅咒
if BeeSpellCD("痛苦诅咒") == 0 and BeeIsRun("/cast 痛苦诅咒", src) and IsSpellInRange("痛苦诅咒", src) == 1 and
    not BeeStringFind("痛苦诅咒", Tbl) and mod_can then
    spell_cast("痛苦诅咒", 1.1, "fss")
